...
interface ConversionsViewProps {
  initialGoals: ConversionGoalData[];
  initialMetrics: {
...
export function ConversionsView({
  ...
  qrOptions,
  campaignOptions,
}: ConversionsViewProps) {
  // Goals & Metrics state
  const [goals, setGoals] = React.useState<ConversionGoalData[]>(initialGoals);
  ...
  const handleStatusToggle = async (id: string, newActive: boolean) => {
    try {
      ...
      toast.success(newActive ? "Goal activated" : "Goal paused");
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this conversion goal? Recorded historical telemetry will be detached.")) {
      return;
    }
    ...
    try {
      ...
      if (deletedGoal?.isActive) {
        setMetrics((prev) => ({
          ...prev,
          activeGoalsCount: Math.max(0, prev.activeGoalsCount - 1),
        }));
      }

      toast.success("Conversion goal removed");
    } catch (err: unknown) {
      console.error("Delete goal error:", err);
      toast.error("Failed to delete conversion goal");
    }
  };
